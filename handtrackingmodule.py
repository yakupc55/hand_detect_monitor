...
class handDetector():
    ...
    def fingersUp(self,lmList=[]):#checking which finger is open 
        
        if len(lmList)<2:
            lmList=self.lmlist
        fingers = []#storing final result
        # Thumb < sign only when  we use flip function to avoid mirror inversion else > sign
        if lmList[self.tipIds[0]][1] > lmList[self.tipIds[0] - 1][1]:#checking x position of 4 is in right to x position of 3
            fingers.append(1)
        else:
            fingers.append(0)
